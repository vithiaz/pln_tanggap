...
export default function AparDetails ({ route, navigation }) {
      ...
      <SafeAreaView style={{ height: '100%', }}>
        <View style={styles.container}>
          ...
          <View style={styles.pageTitleWrapper}>
            <Text style={styles.appTitleText}>Informasi APAR</Text>
...
const PageContentCard = ({ navigation, officeUID, aparData }) => {
  const [desc, setDesc] = useState('')
  const [QRImage, setQRImage] = useState('')

  aparId = aparData.id
  aparDesc = aparData.description
  aparStatus = aparData.status
  ...
  const saveImage = (qr) => {
    ...
    let filename = '/QR_APAR_' + aparId + '_' + Math.floor(date.getTime() + date.getSeconds() / 2) + '.jpeg';
    let PictureDir = fs.dirs.DownloadDir + filename;

    fs.writeFile(PictureDir, qr, 'base64')
    .then(() => {
        RNFetchBlob.android.addCompleteDownload({
            title: 'APAR QR Code id ' + aparId,
  ...
  const handleRegisterApar = () => {
    // try {
    //   set(ref(db, 'apar/' + officeUID + '/' + aparId), {
    //     id: attemptKey,
    //     description: desc,
    //     status: 'Baik',
    //   })
      ...
      <View style={contentCardStyles.contentCard}>
          <View style={styles.containerFillHeight}>
              <View style={contentCardStyles.cardBody}>
                ...
                <View style={contentCardStyles.cardBodyRow}>
                    <Text style={contentCardStyles.cardBodyRowTitle}>Kondisi</Text>
                    <Text style={contentCardStyles.cardBodyRowDesc}>{aparStatus}</Text>
                </View>
                <View style={contentCardStyles.cardBodyRow}>
                ...
                <View style={contentCardStyles.QrRowWrapper}>
                  <View style={{
                    ...
                    <Text >Download atau bagikan QR</Text>
                  </View>
                  <QRCode
                    value={aparId}
